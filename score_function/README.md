...
<picture>
  ...
  <img alt="Edge weight vs opponent goal total for 2- and 3-game sweeps: both lines fall linearly from 1 at a shutout to 0 as the opponent catches up"
       src="weight_function_light.png">
</picture>

*(Generated by matplotlib; the interactive version with hover readouts is
[`weight_function.html`](weight_function.html). The 3-game line is partly
theoretical — a real best-of-three winner playing 3 games concedes one,
so $T_A \le 29$ and $T_B \ge 10$.)*

A dominant sweep is worth up to a full unit of edge weight, a nail-biter
almost nothing, and a totals-tied match contributes zero. There is no
tunable hyperparameter: any global factor $x$ on this weight cancels
exactly, because both the column normalization
...
between narrow and dominant wins reaches the ranking, so the
implementation keeps $x$ pinned at $1$ (the `/scores/x` endpoint still
exists, provably without effect).

A match with no recorded game scores contributes a plain binary edge of
weight $1$. Matches and $d$ are wired through `compute_scores.py`; the
