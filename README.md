...
the base credit, and hyperparameter `x ∈ [0, 10)` (`POST /scores/x`,
0 = plain binary) tunes how much a dominant goal difference adds on top
(see `score_function/README.md`). The bias/weighted/classic switch, the
damping-d slider (default 0.85) and the x input are visible to the
`admin` login only and admin-gated server-side; neither d nor x is
persisted.
